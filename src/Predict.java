...
public class Predict implements src.Command {
    ...
    private static class Word {
        ...
        public void putFollower(String w) {
            this.frequency.put(w, this.frequency.getOrDefault(w, 0) + 1);
        }

        public String predict() {
            if (frequency.isEmpty())
                return null;
            var i = Collections.max(frequency.values());
            var list = this.frequency.keySet().stream().filter(k -> frequency.get(k).equals(i)).toList();
            return list.get(0);
        }
    }
    ...
    public boolean run(Scanner sc) {
        ...
        try{
            ...
            var res = new ArrayList<>(List.of(word));
            while (res.size() < 20) {
                var nextWord = words.get(res.get(res.size() - 1)).predict();
                if (nextWord == null)
                    break;
                res.add(nextWord);
            }
